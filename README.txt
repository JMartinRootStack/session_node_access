INTRODUCTION
------------
This module grants per-session permissions for anonymous users to access nodes
they created.

Yet another simple node access module, this time catering to anonymous users.
It grants temporary (session based) node access permissions to anonymous users
after they create that node.
...
native permission tab) doesn't grant anonymous users to create nodes of a given
tab. The module will check if the anonymous users have this permission and will
...
do so depending on content type.
